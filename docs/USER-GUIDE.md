...
|--------|------|
| **设置...** | 打开偏好设置窗口 |
| **自检与修复** | 一键检查辅助功能权限、Claude/Codex hook 安装状态与配置备份，失败项提供「重新授权 / 重新安装」按钮 |
| **暂停提醒 / 恢复提醒** | 临时关闭提醒（比如你正专注工作不想被打断） |
| **通知历史** | 查看最近的提醒记录，可以清空；点击任意记录可跳转到对应终端窗口（找不到原窗口时退到 Terminal 应用本体） |
| **退出** | 完全退出应用 |
...
| **系统专注模式** | 无需设置。macOS 专注模式（Focus）开启时自动静默：菜单栏红点可见、历史记录可查，但猫不掉下、声音不响。 |
| **关闭提醒后跳转来源应用** | 关闭提醒时自动把 Terminal.app 或 Codex.app 切到最前；Claude 多窗口事件会优先跳到来源 Terminal 窗口。 |
| **超时自动收起** | 默认开启，60 秒。猫挂屏超过设定时长且未处理，自动收起为菜单栏红点（菜单栏猫图标保持提醒态），点菜单栏可重新打开。关闭后猫将一直挂在屏幕上直到手动关闭。 |
| **「稍后」再提醒** | 默认 10 分钟。点击提醒气泡右下角的「稍后」，猫会收起并在此时长后重新弹出，适合暂时没空处理的场景。 |
| **输入时保持迷你** | 默认开启。检测到正在打字时：新提醒以右下角迷你猫出现（不占中央视线），挂着的全尺寸猫也缩为迷你；点击迷你猫展开为完整提醒。使用 CGEventSource 只读查询按键活跃度，无需任何权限、读不到按键内容。 |
| **已屏蔽的会话** | Claude Code 提醒气泡上的「屏蔽」按钮可静音某个特别吵的终端会话（可选「屏蔽全部」或「仅屏蔽完成提醒」，需要确认类继续弹）。屏蔽后在设置 → 通知此列表中管理，可随时解除；条目 7 天后自动失效，防止 tty 编号被新会话复用后继续误屏蔽。 |
...
2. 是否在**免打扰时段**内？
3. 是否开启了**系统专注模式**？（开启时自动静默，菜单栏仍有红点）
4. 是否在**冷却时间**内？（刚提醒过一次，需等冷却结束）
5. **启用提醒**是否打开了？
6. Terminal.app 在运行吗？（必须 Terminal.app 开着才能检测）

如果都正常仍不弹，点菜单栏 → **自检与修复**，逐项排查权限与 hook 状态。
